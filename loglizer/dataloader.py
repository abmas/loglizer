...
def load_SVTFS_TASK(log_file, label_file=None, window='session', train_ratio=0.5, save_df=False):
    """ Load SVTFS Hive controller structured log into train and test data
    ...
    if log_file.endswith('.npz'):
        ...
        assert window == 'session', "Only window=`session` is supported for SVTFS Hivecontroller dataset."
...
def load_SVTFS_HC(log_file, label_file=None, window='session', train_ratio=0.5, save_df=False):
    """ Load SVTFS Hive controller structured log into train and test data
